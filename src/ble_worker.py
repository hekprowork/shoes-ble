...
QUATERNION_CHAR_UUID = "abcdef02-1234-5678-1234-56789abcdef0"
...
class BleSignals(QObject):
    ...
    quaternion_received = Signal(float, float, float, float)
...
class BleWorker(QObject):
    ...
    async def start(self):
        ...
        async with BleakClient(target_device) as client:
            ...
            await client.start_notify(CHAR_UUID, self.sensor_notification_handler)
            await client.start_notify(
                QUATERNION_CHAR_UUID, self.quaternion_notification_handler
            )
            ...
            if client.is_connected:
                await client.stop_notify(QUATERNION_CHAR_UUID)
    ...
    def quaternion_notification_handler(self, sender, data):
        if len(data) == 16:
            w, x, y, z = struct.unpack('<ffff', data)
            self.signals.quaternion_received.emit(w, x, y, z)
